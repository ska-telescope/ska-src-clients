...
# 3. Build ska-src-clients package
cd ../../../ska-src-clients
poetry build

# 4. Install ska-src-clients with all dependencies
cd ../ska-src-web-ui/backend
pip install ../../../ska-src-clients/dist/ska_src_clients-*.whl \
...
The `.env` file in `src/ska-src-web-ui/backend/` is now created automatically by the `setup.sh` script. It will always contain the correct absolute path to your SRCNet config file. You do **not** need to copy or edit `.env` manually unless you want to override defaults.

To set up the backend and dependencies, simply run:
...
- Build and install the `ska-src-clients` package
- Generate the `.env` file with the correct config path
