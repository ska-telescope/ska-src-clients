...
class WebDAVClient3(DataManagementClient):
        ...
        **kwargs
        ...
        options = {
            "webdav_hostname": "{prefix}://{host}:{port}/{path}".format(
                prefix=prefix, host=host, port=port, path=path.lstrip("/")
            ),
    ...
    def download(
        self, progress, progress_args, from_remote_path, to_local_path
    ):
    ...
    def upload(self, from_local_path, to_remote_path):
        ...
        try:
            ...
            self.client.upload_sync(
                local_path=from_local_path, remote_path=to_remote_path
            )
        except WebDavException as e:
